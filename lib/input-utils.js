...
const cursorFromLineCol = (lines = [], targetRow = 0, targetCol = 0) => {
  const r = Math.max(0, Math.min(lines.length - 1, targetRow))
  let offset = 0
  for (let i = 0; i < r; i++) {
  ...
  let curCol = 0
  let charIdx = 0
  while (charIdx < line.length && curCol < targetCol) {
    curCol += visualWidth(line[charIdx])
    if (curCol <= targetCol) {
      charIdx++
    }
  }
...
const handleInputKey = (node, event, ctx) => {
  ...
  if (event.ctrl && key === 'c') {
  ...
  } else if (event.key === 'paste' || event.paste) {
    let pastedText = event.text || event.char || ''
    if (!node.multiline && !node.wrap && node.sanitizePaste !== false) {
      pastedText = pastedText.replace(/[\r\n]+/g, ' ')
    }
    val = val.slice(0, cur) + pastedText + val.slice(cur)
    cur += pastedText.length
  } else if (key === 'up' || key === 'down') {
    const lines = val.split('\n')
    if (lines.length > 1) {
      const loc = findLineCol(lines, cur)
      if (key === 'up') {
        if (loc.row > 0) {
          cur = cursorFromLineCol(lines, loc.row - 1, loc.col)
        } else {
          handled = false
        }
      } else {
        if (loc.row < lines.length - 1) {
          cur = cursorFromLineCol(lines, loc.row + 1, loc.col)
        } else {
          handled = false
        }
    ...
    cur = (node.multiline || val.includes('\n')) ? val.lastIndexOf('\n', cur - 1) + 1 : 0
    ...
    const isExplicitNewline = event.shift || event.alt || (event.ctrl && node.submitMode !== 'ctrl+enter')
    if (isExplicitNewline || (node.submitMode === 'ctrl+enter' && !event.ctrl) || (node.multiline && !node.onSubmit && !event.ctrl)) {
      val = val.slice(0, cur) + '\n' + val.slice(cur)
      cur += 1
    } else if (node.onSubmit) {
      const submittedVal = val
      if (node.clearOnSubmit !== false) {
        val = ''
        cur = 0
      }
      node.onSubmit(ctx, { value: submittedVal, node })
    }

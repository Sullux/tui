...
const onMeasure = (node, constraints) => {
  ...
  if (node.multiline) {
    const lines = node.wrap ? wrapText(val, constraints.maxWidth) : val.split('\n')
    const width = lines.reduce((max, l) => Math.max(max, visualWidth(l)), 0)
    return { width: Math.min(constraints.maxWidth, Math.max(1, width)), height: Math.max(1, lines.length) }
  }

...
const onRender = (node, grid) => {
  ...
  if (activeCaret !== false && activeCaret !== null && activeCaret !== undefined) {
    const defaultMode = (typeof activeCaret === 'object' && activeCaret.char) ? 'block' : 'invert'
    const caretDef = typeof activeCaret === 'string'
      ? { type: 'caret', char: activeCaret, mode: 'block' }
      : (typeof activeCaret === 'object' ? { type: 'caret', mode: activeCaret.mode || defaultMode, ...activeCaret } : { type: 'caret', mode: 'invert' })
    caretDef.box = {
      x: caretX,
      y: caretY,

...
const onRender = (node, grid) => {
  ...
  // Paint Caret if focused, or unfocusedCaret if not focused
  const activeCaret = node.hasFocus ? node.caret : node.unfocusedCaret
  if (activeCaret !== false && activeCaret !== null && activeCaret !== undefined) {
    const defaultMode = (typeof activeCaret === 'object' && (activeCaret.char || activeCaret.bg)) ? undefined : 'invert'
    const caretDef = typeof activeCaret === 'string'
      ? { type: 'caret', char: activeCaret, mode: 'block' }
      : (typeof activeCaret === 'object' ? { type: 'caret', ...(defaultMode ? { mode: defaultMode } : {}), ...activeCaret } : { type: 'caret', mode: 'invert' })
    caretDef.box = {
      x: caretX,
      y: caretY,

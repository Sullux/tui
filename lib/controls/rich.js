  ...
  children.forEach(child => {
    if (child.type === 'text' || typeof child.text === 'string') {
      ...
      lines.forEach((lineText, lineIdx) => {
        ...
        // If there's another line following a '\n', push a line-break spacer to advance Wrap to the next line
        if (lineIdx < lines.length - 1) {
          const breakSpacer = compiler.compileNode({
            type: 'text',
            text: '',
            width: maxWidth,
            height: 0,
